...
ODE 是一个面向 agent 协作的机会发现引擎，也提供命令行接口。它帮助独立开发者和小团队系统化地发现、评估和筛选商业机会。
...
推荐用法是在 agent 里直接用自然语言描述目标。你不需要记住所有命令；agent 会根据需求调用 `ode` 命令、读取数据源目录、分析赚钱案例、更新文档或生成报告。

可以这样说：

```text
帮我扫描最近一个月国内外已经赚到钱的 AI 产品案例，先按证据等级筛选，再判断哪些适合我做。

我想做一个面向独立开发者的工具，帮我创建机会、扫描信号、评估分数，并生成报告。

每天帮我关注中国和海外主流平台上的机会，重点看能证明有人付费的案例，不要只看热度。

用我的画像筛选这些机会，但不要过早过滤掉异类机会，保留高 discovery value 的 watchlist。

检查当前数据源覆盖是否缺主流平台，缺的话先补 source registry，不要直接写爬虫。
```

Agent 通常会按这个顺序工作：

1. 先看 `sources`，确认数据源覆盖和当前哪些源真的会自动扫描。
2. 再看 `cases`，找已经赚到钱的案例，并区分收入、GMV、融资、流量和宣传。
3. 如果某个方向值得继续，创建 opportunity，跑 scan/eval/insights/lens/report。
4. 如果需要长期跟踪，再把固定命令接到系统计划任务、CI，或 Codex automation。

只有当你想手动运行或调试时，才需要直接使用下面的命令。
