"""Product Hunt Atom feed source adapter."""

from __future__ import annotations

import logging
import time

from ode.tools.sources.common import append_note, clean_feed_text, strength_from_position

logger = logging.getLogger(__name__)

SOURCE_ID = "producthunt_feed"
DEFAULT_FEED_URL = "https://www.producthunt.com/feed"
RETRY_STATUSES = {429, 500, 502, 503, 504}


def scan(limit: int = 30, feed_url: str = DEFAULT_FEED_URL,
         source_notes: list[dict] | None = None) -> list[dict]:
    """Scan Product Hunt's public Atom feed for recent launches.

    Product Hunt's official GraphQL API needs an access token. The public
    feed is intentionally treated as a weak launch/solution-side signal: it
    helps spot what makers are shipping, but does not prove revenue or pain.
    """

    try:
        import requests
        import xml.etree.ElementTree as ET
    except ImportError:
        logger.warning("requests not installed: pip install requests")
        append_note(
            source_notes,
            source_id=SOURCE_ID,
            status="unavailable",
            reason="missing_dependency",
            dependency="requests",
        )
        return []

    results = []
    headers = {"User-Agent": "ode-opportunity-engine/0.1"}
    ns = {"atom": "http://www.w3.org/2005/Atom"}

    try:
        resp, attempts = _get_with_retry(requests, feed_url, headers=headers, timeout=10)
        if resp.status_code != 200:
            logger.warning("Product Hunt feed returned %s", resp.status_code)
            append_note(
                source_notes,
                source_id=SOURCE_ID,
                status="failed",
                reason="http_status",
                http_status=resp.status_code,
                attempts=attempts,
            )
            return results

        root = ET.fromstring(resp.text)
        entries = root.findall("atom:entry", ns)
        if not entries:
            append_note(
                source_notes,
                source_id=SOURCE_ID,
                status="empty",
                attempts=attempts,
            )

        for position, entry in enumerate(entries[:limit]):
            title_el = entry.find("atom:title", ns)
            link_el = entry.find("atom:link", ns)
            content_el = entry.find("atom:content", ns)
            published_el = entry.find("atom:published", ns)
            updated_el = entry.find("atom:updated", ns)
            id_el = entry.find("atom:id", ns)

            title = clean_feed_text(title_el.text if title_el is not None else "")
            url = link_el.get("href", "") if link_el is not None else ""
            snippet = ""
            if content_el is not None:
                snippet = clean_feed_text(" ".join(content_el.itertext()))

            results.append({
                "source_id": SOURCE_ID,
                "source": "producthunt",
                "keyword": title,
                "title": title,
                "url": url,
                "summary": snippet,
                "score": 0,
                "comments": 0,
                "rank": position + 1,
                "strength": strength_from_position(position, strong_before=5, medium_before=15),
                "momentum": 0,
                "published": published_el.text if published_el is not None else "",
                "updated": updated_el.text if updated_el is not None else "",
                "external_id": id_el.text if id_el is not None else "",
            })
    except ET.ParseError as exc:
        logger.warning("Product Hunt feed parse failed: %s", exc)
        append_note(
            source_notes,
            source_id=SOURCE_ID,
            status="failed",
            reason="malformed_xml",
            error=str(exc),
        )
    except Exception as exc:
        logger.warning("Product Hunt scan failed: %s", exc)
        append_note(
            source_notes,
            source_id=SOURCE_ID,
            status="failed",
            reason="request_error",
            error=str(exc),
        )

    return results


def _get_with_retry(requests_module, url: str, *, headers: dict, timeout: int):
    attempts = 0
    response = None
    for attempts in range(1, 3):
        response = requests_module.get(url, headers=headers, timeout=timeout)
        if response.status_code not in RETRY_STATUSES:
            break
        if attempts == 1:
            time.sleep(0.1)
    return response, attempts
